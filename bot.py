...
async def text_cancel(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Все действия отменены.", reply_markup=main_kb())
...
async def adm_promo(message: types.Message, command: CommandObject, db: Database):
    ...
    except: await message.answer("❌ Ошибка. Формат: /add_promo <КОД> <ЗВЕЗДЫ> <ЛИМИТ>")
...
async def adm_get_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ Теперь введи через запятую: Имя, Рейтинг, Позиция, Клуб, Редкость")
...
async def adm_save_card(message: types.Message, state: FSMContext, db: Database):
    try:
        ...
        await message.answer("✅ Карта успешно добавлена в базу!")
    except: await message.answer("❌ Ошибка в формате данных.")
    await state.clear()
...
async def cmd_bonus(message: types.Message, db: Database):
    ...
    if u.get('last_bonus') and datetime.now() < u['last_bonus'] + timedelta(hours=24):
        ...
        return await message.answer(f"⏳ Бонус будет доступен через {diff.seconds // 3600}ч. {(diff.seconds // 60) % 60}м.")
    ...
    await message.answer(f"🎁 Ты получил бонус {val} 🌟!")

@dp.message(F.text == "🏷 Промокод")
async def promo_start(message: types.Message, state: FSMContext):
    await state.set_state(PromoState.waiting_for_code)
    await message.answer("🏷 Введи промокод или нажми кнопку отмены ниже:", reply_markup=cancel_inline())
...
async def promo_use(message: types.Message, state: FSMContext, db: Database):
    ...
    if not promo or promo['current_uses'] >= promo['max_uses']:
        return await message.answer("❌ Код не существует или лимит исчерпан.", reply_markup=main_kb())
    
    used = await db.pool.fetchval("SELECT 1 FROM used_promos WHERE user_id = $1 AND code = $2", message.from_user.id, code)
    if used: return await message.answer("❌ Ты уже использовал этот код!")
    ...
    await message.answer(f"✅ Успешно! +{promo['stars']} 🌟")
    await state.clear()
...
async def guess_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text("🧩 Введи свою ставку (от 100 🌟):", reply_markup=cancel_inline())
...
async def guess_logic(message: types.Message, state: FSMContext, db: Database):
    ...
    await state.update_data(correct=card['name'], bet=bet, cid=card['card_id'], opts=options)
    kb = [[InlineKeyboardButton(text=o, callback_data=f"ans_{i}")] for i, o in enumerate(options)]
...
async def guess_check(callback: types.CallbackQuery, state: FSMContext, db: Database):
    ...
    if chosen == data['correct']:
        ...
        txt = f"✅ Верно! Это {card['name']}. Выигрыш: {win} 🌟"
    else: txt = f"❌ Нет, это был {card['name']}."
    
    await callback.message.answer_photo(card['photo_id'], caption=txt)
    await callback.message.delete()
    await state.clear()

@dp.message(F.text == "🔄 Трейд")
async def trade_init(message: types.Message):
    kb = [[InlineKeyboardButton(text="📤 Создать код", callback_data="tr_create")],
          [InlineKeyboardButton(text="📥 Ввести код", callback_data="tr_join")]]
    await message.answer("🔄 ОБМЕН КАРТАМИ\nВыберите действие:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def tr_create_pg(callback: types.CallbackQuery, db: Database):
    ...
    if not cards: return await callback.answer("У тебя нет карт!", show_alert=True)
    kb = [[InlineKeyboardButton(text=f"{c['name']}", callback_data=f"trgen_{c['card_id']}")] for c in cards]
    await callback.message.edit_text("Выбери карту для обмена:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def tr_gen_final(callback: types.CallbackQuery, db: Database):
    ...
    await callback.message.edit_text(f"✅ Код создан: `{code}`\nОтправь его другу.")

@dp.callback_query(F.data == "tr_join")
async def tr_join_input(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("📥 Введи код обмена:", reply_markup=cancel_inline())
...
async def tr_join_logic(message: types.Message, state: FSMContext, db: Database):
    ...
    if not tr or tr['user_a'] == message.from_user.id: return await message.answer("❌ Код неверный.")
...
async def tr_send_to_a(callback: types.CallbackQuery, state: FSMContext, db: Database):
    ...
    await bot.send_message(tr['user_a'], f"🔄 ПРЕДЛОЖЕНИЕ ОБМЕНА!\nВам предлагают карту за кодом {code}.", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.message.edit_text("⏳ Запрос отправлен владельцу.")
    await state.clear()
...
async def tr_accept(callback: types.CallbackQuery, db: Database):
    ...
    tr = await db.pool.fetchrow("SELECT * FROM active_trades WHERE code = $1", code)
    if not tr: return await callback.message.edit_text("❌ Обмен более не активен.")
    
    # Свап карт в инвентаре
    async with db.pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute("DELETE FROM inventory WHERE user_id = $1 AND card_id = $2", tr['user_a'], tr['card_a'])
            await conn.execute("DELETE FROM inventory WHERE user_id = $1 AND card_id = $2", int(ub_id), int(cb_id))
            await conn.execute("INSERT INTO inventory (user_id, card_id) VALUES ($1, $2), ($3, $4)", tr['user_a'], int(cb_id), int(ub_id), tr['card_a'])
            await conn.execute("DELETE FROM active_trades WHERE code = $1", code)
            
    await callback.message.edit_text("✅ Обмен завершен успешно!")
    await bot.send_message(int(ub_id), "✅ Твой обмен был принят!")
...
async def cmd_start(message: types.Message, command: CommandObject, db: Database):
    ...
    await db.pool.execute("INSERT INTO users (user_id, username, referrer_id) VALUES ($1, $2, $3) ON CONFLICT (user_id) DO UPDATE SET username = $2", 
                          message.from_user.id, message.from_user.username or message.from_user.first_name, ref)
...
async def get_card(message: types.Message, db: Database):
    ...
    if u.get('last_drop') and datetime.now() < u['last_drop'] + timedelta(hours=cd):
        ...
        return await message.answer(f"⏳ Жди {diff.seconds // 3600}ч. {(diff.seconds // 60) % 60}м.")
